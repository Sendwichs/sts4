...
<body>
			...
			<tr>
				<td>
					...
					%> <b>
						...
						<%=name%>(<%=dto.getMyid()%>)
					</b> <span class="answer" style="cursor: pointer;" num="<%=dto.getNum()%>">
						댓글<%=alist.size()%></span> <span class="likes" style="margin-left: 20px; cursor: pointer;" num="<%=dto.getNum()%>">추천</span> <span class="chu"><%=dto.getChu()%></span> <span class="glyphicon glyphicon-heart" style="color: red; font-size: 0px;"></span> <%
 //로그인한 아이디와 글을쓴아이디가 같을경우에만 수정삭제 보이게..(자기글만)
 if (loginok != null && dto.getMyid().equals(myid)) {
 %> | <a href="index.jsp?main=guest/updateform.jsp?num=<%=dto.getNum()%>&currentPage=<%=currentPage%>" style="color: black;">수정</a> | <a href="guest/delete.jsp?num=<%=dto.getNum()%>&currentPage=<%=currentPage%>" style="color: black;">삭제</a> <%
 }
 %> <span class="day"><%=sdf.format(dto.getWriteday())%></span> <!-- 댓글 -->
					...
					<div class="answer">
						...
						<div class="answerform">
							<form action="guest/answerinsert.jsp" method="post">
								...
								<table>
									<tr>
										<td><textarea style="width: 470px; height: 70px;" name="content" required="required" class="form-control"></textarea></td>
										<td>
						...
						<div class="answerlist">
							<table class="table" style="width: 500px;">
								...
								%>
								<tr>
									<td width="60" align="left"><span class="glyphicon glyphicon-user"></span></td>
									<td>
										<%
										//작성자명얻기
 ...
 %> <span class="adel glyphicon glyphicon-trash" idx="<%=adto.getIdx()%>" style="font-size: 12pt; color: green; margin-left: 10pt; cursor: pointer;"></span> <%
 }
